...
export class RegulatorComponent implements OnInit {
  ...
  RegulatorBranch = new FormControl('', Validators.required);
  RegulatorAdress = new FormControl('', Validators.required);
  ...
  constructor(public serviceRegulator: RegulatorService, fb: FormBuilder) {
    this.myForm = fb.group({
      ...
      RegulatorBranch: this.RegulatorBranch,
      RegulatorAdress: this.RegulatorAdress
    });
  };
  ...
  addParticipant(form: any): Promise<any> {
    this.participant = {
      $class: 'org.prototype.blockchainhalal.Regulator',
      'RegulatorID': this.RegulatorID.value,
      'RegulatorName': this.RegulatorName.value,
      'RegulatorBranch': this.RegulatorBranch.value,
      'RegulatorAdress': this.RegulatorAdress.value
    };

    this.myForm.setValue({
      'RegulatorID': null,
      'RegulatorName': null,
      'RegulatorBranch': null,
      'RegulatorAdress': null
    });
  ...
  getForm(id: any): Promise<any> {
    ...
    .then((result) => {
      ...
      if (result.RegulatorBranch) {
        formObject.RegulatorBranch = result.RegulatorBranch;
      } else {
        formObject.RegulatorBranch = null;
      }

      if (result.RegulatorAdress) {
        formObject.RegulatorAdress = result.RegulatorAdress;
      } else {
        formObject.RegulatorAdress = null;
      }
